...
#plot actual correlation distribution, not the z-score
plotSubtype<- PANGEA_McGrailRSR_Output%>%
  arrange(desc(McGrailRSR_Corr)) %>%
  ggplot(aes(x=reorder(Drug_CellLine, -McGrailRSR_Corr), y=McGrailRSR_Corr)) +
  geom_bar(stat="identity", aes(fill=Cell_Line)) #+ 
  #scale_fill_manual(values = cols, na.value = "grey")  
plotFinal<-plotSubtype + labs(title = "Distribution of McGrail RSR Correlations All ", subtitle = "PANGEA (80%)") +
  xlab("") +
  ylab("McGrail RSR Correlation") + 
  guides(fill=guide_legend(title="Cell Line")) + 
  theme(#axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
        axis.title.x=element_blank(),
        axis.ticks.x =element_blank(),
...
#output file
View(drugLibrary)
PANGEA_McGrailPARP_Output_joinDrug<-left_join(PANGEA_McGrailPARP_Output, drugLibrary) %>%
  select(Drug, Cell_Line, Dose, Time, McGrailPARP_Corr, McGrailPARP_Z_Score, class, pathway, target, fda,Cmap_clinical)
View(PANGEA_McGrailPARP_Output_joinDrug)
#write to file
write_csv(PANGEA_McGrailPARP_Output_joinDrug, here("output", "MinePerturbed_PANGEA", "PANGEA_McGrailPARPness_TopHits.csv"))
